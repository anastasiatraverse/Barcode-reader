...
Label(root, text='Hello! Please enter the file name\n'
            'Then choose method\n'
            'Click Enter!').grid(row = 1, column=1, columnspan=3)



Button(root, text="Create barcode", command = get_create).grid(row=2, column=1)
Button(root, text="Decode barcode (method 1)", command =decode_m1).grid(row=2, column=2)
Button(root, text="Decode barcode (method 2)", command = decode_m2).grid(row=2, column=3)
root.mainloop()
